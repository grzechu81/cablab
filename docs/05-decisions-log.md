...
| 35 | With a grooved HDF back, the **top** panel is shortened to `depth - grooveOffsetFromBack - grooveWidth`; the bottom stays full depth | The side-panel groove is a vertical channel — a full-depth top panel caps it and the HDF can't be slid in after assembly. Ending the top at the groove front keeps the channel open. The bottom supports the HDF, so it stays full |
